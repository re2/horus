const scriptName = '顺丰速运';
const getCookieRegex = /^https?:\/\/sf-integral-sign-in\.weixinjia\.net\/app\/init/;
const appCookieKey = 'sf_checkin_cookie';
let magicJS = MagicJS(scriptName, "DEBUG");

function Login(cookie){
  return new Promise((resolve, reject)=>{
    let Options = {
      url: 'https://sf-integral-sign-in.weixinjia.net/app/init',
      headers: {
        ...
        "Origin": "https://sf-integral-sign-in.weixinjia.net",
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 mediaCode=SFEXPRESSAPP-iOS-ML"
      },
      body: ''
    }
    magicJS.post(Options, (err, resp, data)=>{
      if (err){
        magicJS.logError(`登录失败，请求异常：${err}`);
        reject('❌登录失败，请求异常，请查阅日志！');
      }
      else{
        try{
          let obj = JSON.parse(data);
          if (obj.code === 0 && obj.data.isLogin === true){
            magicJS.logInfo(`登录成功，等级${obj.data.member_info.gradeVal}，连续签到${obj.data.check_count}天!`)
            resolve(['登录成功', obj.data.check_count, obj.data.member_info.gradeVal]);
          }
          else{
            magicJS.logError(`签到失败，响应异常：${data}`);
            reject('❌登录失败，响应异常，请查阅日志！');
          }
        }
        catch(err){
          magicJS.logError(`签到失败，执行异常：${err}，接口响应：${data}`);
          reject('❌登录失败，执行异常，请查阅日志！');
        }
      }
    })
  })
...
function CheckIn(cookie){
  return new Promise((resolve, reject)=>{
    let checkinOptions = {
      ...
      body: `date=${magicJS.formatTime(new Date(), "yyyy-MM-dd")}`
    }
      ...
      if (err){
        ...
        try{
          ...
          if (obj.code === 0 && obj.msg == 'success'){
            magicJS.logInfo(`签到成功，连续签到${obj.data.check_count}，获取积分${obj.data.prize.integral}!`)
            resolve(['🎉签到成功', obj.data.check_count, obj.data.prize.integral]);
          }
          else if(obj.code === -1 && obj.msg == 'ALREADY_CHECK'){
            magicJS.logError(`重复签到：${data}`);
            resolve(['🎉重复签到', null, null]);
          }
          else{
            magicJS.logError(`签到失败，响应异常：${data}`);
...
;(async()=>{
  if (magicJS.isRequest && getCookieRegex.test(magicJS.request.url)){
    ...
    let hisCookie = magicJS.read(appCookieKey);
    if (cookie !== hisCookie){
      magicJS.write(appCookieKey, cookie);
    ...
    let subTitle = "";
    let content = "";
    let cookie = magicJS.read(appCookieKey);
    if (!!!cookie){
      magicJS.logWarning('没有读取到Cookie，请先从App中获取一次Cookie！');
      magicJS.notify(`❓没有读取到有效Cookie，请先从App中获取Cookie!!`);
    }
    else{
      let [loginErr,[, beforeCheckCount,gradeVal]] = await  magicJS.attempt(magicJS.retry(Login, 1, 1000)(cookie), ['登录失败', ['登录失败', null, null]]);
      if (loginErr){
        subTitle = loginErr;
      }
      else{
        let [checkInErr,[resultStr, afterCheckCount, integral]] = await magicJS.attempt(magicJS.retry(CheckIn, 1, 1000)(cookie), ['签到失败', ['签到失败', null, null]]);
      
        if (checkInErr){
          subTitle = checkInErr;
        }
        else{
          subTitle = `${resultStr}，等级${gradeVal}，连续签到${afterCheckCount? afterCheckCount: beforeCheckCount}天！`;
          if (integral) content = `本次签到获得${integral}积分`;
        }
      }
      // 通知
